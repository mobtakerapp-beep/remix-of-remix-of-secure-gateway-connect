  ...
  return (
    <header className="border-b-4 border-double border-primary px-10 pb-5 pt-8 print:px-0 print:pt-0">
      ...
      <div className="mt-5 grid gap-3 text-sm sm:grid-cols-3 print:grid-cols-3">
        <p className="rounded-xl border border-dashed border-primary/40 px-3 py-2">{ar ? "الاسم" : "Name"}: ______________</p>
        <p className="rounded-xl border border-dashed border-primary/40 px-3 py-2">{ar ? "الصف" : "Class"}: __________</p>
        <p className="rounded-xl border border-dashed border-primary/40 px-3 py-2">{ar ? "التاريخ" : "Date"}: __________</p>
      </div>
    </header>
  );
}

function Sheet({ pkg, answerKey, teacher, hideMascots, t }: { pkg: LessonPackage; answerKey: boolean; teacher: string; hideMascots: boolean; t: Dict }) {
  const ar = pkg.language === "ar";
  const dir = ar ? "rtl" : "ltr";
  const num = (n: number | string) => fmtNum(n, pkg.numerals);
  return (
    <div dir={dir} className="mx-auto w-full max-w-[820px] overflow-hidden rounded-3xl border border-border bg-card text-card-foreground shadow-[var(--shadow-soft)] print:max-w-none print:rounded-none print:border-0 print:shadow-none">
      <SheetHeader pkg={pkg} ar={ar} hideMascots={hideMascots} />
      <div className="px-10 pb-10 print:px-0 print:pb-0">
        {pkg.mcqs.length > 0 && (
          <section className="mt-7">
            <div className="break-inside-avoid">
              <h3 className="flex items-center gap-2 font-display text-base font-extrabold text-primary"><span className="grid size-7 place-items-center rounded-full border border-primary bg-primary text-sm text-primary-foreground">{num(1)}</span>{ar ? "اختر الإجابة الصحيحة" : "Multiple choice"}</h3>
              <ol className="mt-3 space-y-4 text-sm"><McqItem m={pkg.mcqs[0]!} index={0} answerKey={answerKey} pkg={pkg} /></ol>
            </div>
            {pkg.mcqs.length > 1 && <ol className="mt-3 space-y-4 text-sm" start={2}>{pkg.mcqs.slice(1).map((m, i) => <McqItem key={m.id} m={m} index={i + 1} answerKey={answerKey} pkg={pkg} />)}</ol>}
          </section>
        )}
        {pkg.trueFalse.length > 0 && (
          <section className="mt-7">
            <div className="break-inside-avoid">
              <h3 className="flex items-center gap-2 font-display text-base font-extrabold text-primary"><span className="grid size-7 place-items-center rounded-full border border-primary bg-primary text-sm text-primary-foreground">{num(2)}</span>{ar ? "صح أم خطأ" : "True or False"}</h3>
              <ol className="mt-3 space-y-2 text-sm"><TfItem tf={pkg.trueFalse[0]!} index={0} answerKey={answerKey} pkg={pkg} /></ol>
            </div>
            {pkg.trueFalse.length > 1 && <ol className="mt-3 space-y-2 text-sm" start={2}>{pkg.trueFalse.slice(1).map((tf, i) => <TfItem key={tf.id} tf={tf} index={i + 1} answerKey={answerKey} pkg={pkg} />)}</ol>}
          </section>
        )}
    ...
    <div className="space-y-4">
      <div className="no-print flex flex-wrap items-end justify-between gap-4 rounded-3xl border border-border bg-card p-4">
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex items-center gap-3"><Switch id="key" checked={answerKey} onCheckedChange={setAnswerKey} /><Label htmlFor="key">{t.showKey}</Label></div>
          <div className="flex items-center gap-3"><Switch id="hide-mascots" checked={hideMascots} onCheckedChange={toggleHide} /><Label htmlFor="hide-mascots">{lang === "ar" ? "إخفاء الرسوم الكرتونية" : "Hide cartoon images"}</Label></div>
          <div className="min-w-56"><Label htmlFor="teacher-worksheet">{t.teacherName}</Label><Input id="teacher-worksheet" value={teacher} onChange={(e) => setTeacher(e.target.value)} placeholder={t.teacherPlaceholder} className="mt-1.5 rounded-xl" /></div>
        </div>
        <Button onClick={downloadPdf} disabled={exporting} className="rounded-full gradient-hero text-primary-foreground"><Printer className="mr-2 size-4" /> {exporting ? t.preparingPdf : t.print}</Button>
      </div>
      <p className="no-print flex items-start gap-2 rounded-2xl bg-secondary/60 px-4 py-3 text-xs text-secondary-foreground"><Info className="mt-0.5 size-4 shrink-0" /> {t.pdfTip}</p>
      <div ref={sheetRef} className="worksheet-export bg-card"><Sheet pkg={pkg} answerKey={answerKey} teacher={teacher} hideMascots={hideMascots} t={t} /></div>
    </div>
  );
}
