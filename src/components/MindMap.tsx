...
export function buildBranches(pkg: LessonPackage): Branch[] {
  if (pkg.summarySections.length) {
    return pkg.summarySections.slice(0, 6).map((section) => ({
      ...
      children: section.points.slice(0, 6).flatMap((point) => [point.text, ...point.subPoints.slice(0, 3)]),
    }));
  }
...
/** Responsive HTML mind map with the same math renderer used everywhere else. */
export function MindMap({ pkg }: { pkg: LessonPackage }) {
  ...
  return (
    <section dir={ar ? "rtl" : "ltr"} className="mind-map w-full overflow-hidden rounded-3xl border border-primary/20 bg-card p-4 sm:p-6">
      <div className="mx-auto flex max-w-lg items-center justify-center gap-3 rounded-2xl bg-primary px-5 py-4 text-center text-primary-foreground shadow-[var(--shadow-soft)] print:shadow-none">
        ...
        <div>
          ...
          <h4 className="font-display text-base font-extrabold leading-7"><MathText text={pkg.title} numerals={pkg.numerals} /></h4>
        </div>
      </div>
            ...
            <article key={`${b.label}-${i}`} className="flex min-w-0 break-inside-avoid flex-col items-center">
              ...
              <div className={`h-full w-full rounded-2xl border-2 p-4 ${TONES[i % TONES.length]}`}>
                <div className="flex items-start gap-2">
                  <span className="grid size-8 shrink-0 place-items-center rounded-xl bg-card text-primary">{i % 3 === 0 ? <Lightbulb className="size-4" /> : i % 3 === 1 ? <BookOpen className="size-4" /> : <Tag className="size-4" />}</span>
                  <p className="min-w-0 text-sm font-extrabold leading-7 text-foreground"><MathText text={b.label} numerals={pkg.numerals} /></p>
                </div>
                {b.children.length > 0 && (
                  <ul className="mt-3 space-y-2 border-t border-border pt-3 text-xs text-muted-foreground">
                    {b.children.map((c, k) => <li key={`${c}-${k}`} className="flex items-start gap-1.5"><span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-emerald" /><span className="leading-6"><MathText text={c} numerals={pkg.numerals} /></span></li>)}
                  </ul>
                )}
              </div>
            </article>
