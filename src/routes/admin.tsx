...
export const Route = createFileRoute("/admin")({
  head: () => ({ meta: [{ title: "لوحة التحكم — ملخصي" }, { name: "description", content: "إدارة أكواد التفعيل والاشتراكات في ملخصي." }] }),
  component: AdminPage,
});
...
function periodLabel(days: number, note: string | null | undefined, ar: boolean) {
...
function AdminPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!admin) { try { admin = await isAdminClient(); } catch {} }
      setAllowed(admin);
  ...
  const generate = async (override?: { kind: "paid" | "gift"; plan: "standard" | "premium"; days: number }) => {
    setBusy(true);
    try {
      const kind = override?.kind ?? codeKind;
      const days = override?.days ?? (kind === "gift" ? Math.max(1, Math.min(3650, giftDays)) : durationDays);
  ...
  const removeCode = async (row: CodeRow) => {
    if (!window.confirm(ar ? `هل تريدين حذف الكود ${row.code} نهائيًا؟` : `Delete code ${row.code} permanently?`)) return;
    setBusy(true);
    try {
  ...
  const exportCsv = () => {
    ...
    const lines = filtered.map((r) => [r.code, planLabel(r.plan, r.note, ar), periodLabel(r.durationDays, r.note, ar), r.durationDays, r.usedCount, r.maxUses, `"${(r.note ?? "").replace(/"/g, '""')}"`, r.active ? (ar ? "فعّال" : "Active") : (ar ? "موقوف" : "Disabled"), dateLabel(r.createdAt, ar)].join(","));
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <Card className="rounded-3xl border-primary/30 bg-primary/5 p-5">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            <Button className="rounded-full gradient-hero text-primary-foreground" disabled={busy} onClick={() => void generate({ kind: "paid", plan: "standard", days: 30 })}>{ar ? "شهري عادي — $7" : "Monthly Standard — $7"}</Button>
            <Button variant="outline" className="rounded-full" disabled={busy} onClick={() => void generate({ kind: "paid", plan: "premium", days: 30 })}>{ar ? "شهري مميز — $15" : "Monthly Premium — $15"}</Button>
            <Button variant="outline" className="rounded-full" disabled={busy} onClick={() => void generate({ kind: "paid", plan: "standard", days: 365 })}>{ar ? "سنوي عادي — $50" : "Yearly Standard — $50"}</Button>
            <Button variant="outline" className="rounded-full" disabled={busy} onClick={() => void generate({ kind: "paid", plan: "premium", days: 365 })}>{ar ? "سنوي مميز — $100" : "Yearly Premium — $100"}</Button>
            <Button variant="outline" className="rounded-full" disabled={busy} onClick={() => void generate({ kind: "gift", plan: "standard", days: Math.max(1, Math.min(3650, giftDays)) })}><Plus className="me-2 size-4" />{ar ? "هدية" : "Gift"}</Button>
          </div>
        </Card>
        ...
        <Card className="rounded-3xl p-5">
          ...
          <p className="mt-2 text-xs text-muted-foreground">{ar ? "الهدية تسمح بالنص والصور فقط، وPDF والفيديو مقفولين." : "Gift access includes text and images only; PDF and video are locked."}</p>
        </Card>
