      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <Card className="rounded-3xl p-5">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1.5"><Label>{ar ? "الخطة" : "Plan"}</Label><select value={`${plan}-${durationDays >= 365 ? "yearly" : "monthly"}`} disabled={codeKind === "gift"} onChange={(e) => { const [nextPlan, period] = e.target.value.split("-"); setPlan(nextPlan as "standard" | "premium"); setDurationDays(period === "yearly" ? 365 : 30); }} className="h-10 w-full rounded-xl border border-input bg-background px-3 text-sm"><option value="standard-monthly">{ar ? "شهري عادي — $7" : "Monthly Standard — $7"}</option><option value="standard-yearly">{ar ? "سنوي عادي — $50" : "Yearly Standard — $50"}</option><option value="premium-monthly">{ar ? "شهري مميز — $15" : "Monthly Premium — $15"}</option><option value="premium-yearly">{ar ? "سنوي مميز — $100" : "Yearly Premium — $100"}</option></select></div>
