...
export const redeemCode = createServerFn({ method: "POST" })
  .middleware([requireAppAuth])
  .validator((input: unknown) => z.object({ code: z.string().min(4).max(64), device: z.string().max(200).optional() }).parse(input))
  .handler(async ({ data, context }) => {
    const { supabaseAdmin } = await import("@/lib/supabase-admin.server");
    const rawInput = data.code.trim();
    const cleanChars = rawInput.replace(/[^a-zA-Z0-9]/g, "");
    const formattedCode = cleanChars.length === 12 ? `${cleanChars.slice(0, 4)}-${cleanChars.slice(4, 8)}-${cleanChars.slice(8, 12)}` : rawInput;
    const { data: row, error: fetchError } = await supabaseAdmin.from("activation_codes").select("*").ilike("code", formattedCode).maybeSingle();
    ...
    if (!neverUsed && row.expires_at && new Date(row.expires_at) < new Date()) return { ok: false as const, reason: "expired" };
    const { data: mine } = await supabaseAdmin.from("code_redemptions").select("id").eq("code_id", row.id).eq("user_id", context.userId).maybeSingle();
    ...
    if (!mine) {
      ...
      await supabaseAdmin.from("code_redemptions").insert({ code_id: row.id, user_id: context.userId, device_fingerprint: data.device ?? null });
      await supabaseAdmin.from("activation_codes").update(neverUsed && row.max_uses <= 1 ? { used_count: (row.used_count ?? 0) + 1, expires_at: codeExpiry.toISOString() } : { used_count: (row.used_count ?? 0) + 1 }).eq("id", row.id);
    }
    ...
    const payload = { user_id: context.userId, plan: normalizedPlan, status: "active", expires_at: expires.toISOString(), generations_used: 0, reset_at: new Date().toISOString() };
    const { data: existing } = await supabaseAdmin.from("subscriptions").select("id").eq("user_id", context.userId).maybeSingle();
    if (existing) await supabaseAdmin.from("subscriptions").update(payload).eq("user_id", context.userId);
...
export const adminCreateCodes = createServerFn({ method: "POST" })
  .middleware([requireAppAuth])
  .validator((input: unknown) => z.object({ count: z.number().int().min(1).max(50), plan: z.enum(["standard", "premium", "monthly", "yearly"]), durationDays: z.number().int().min(1).max(3650), maxUses: z.number().int().min(1).max(1000), note: z.string().max(200).optional(), notes: z.array(z.string().max(200)).max(50).optional() }).parse(input))
  .handler(async ({ data, context }) => {
    await assertAdmin(context.userId, getClaimEmail(context.claims), context.supabase);
    const { supabaseAdmin } = await import("@/lib/supabase-admin.server");
    const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    const gen = () => { const bytes = new Uint8Array(12); crypto.getRandomValues(bytes); const raw = Array.from(bytes, (b) => alphabet[b % alphabet.length]).join(""); return `${raw.slice(0, 4)}-${raw.slice(4, 8)}-${raw.slice(8, 12)}`; };
    const normalizedPlan = data.plan === "standard" || data.plan === "monthly" ? "standard" : "premium";
    const rows = Array.from({ length: data.count }, (_unused, i) => ({ code: gen(), plan: normalizedPlan, duration_days: data.durationDays, max_uses: data.maxUses, note: data.notes?.[i]?.trim() || data.note?.trim() || null, created_by: context.userId }));
...
export const adminListRedemptions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<RedemptionRow[]> => {
    await assertAdmin(context.userId, getClaimEmail(context.claims), context.supabase);
    const { supabaseAdmin } = await import("@/lib/supabase-admin.server");
    const { data: redemptions } = await supabaseAdmin.from("code_redemptions").select("id, user_id, device_fingerprint, created_at, activation_codes(code, plan, note, duration_days)").order("created_at", { ascending: false }).limit(300);
    ...
    if (userIds.length > 0) {
      ...
      } catch (e) { console.error("[adminListRedemptions] listUsers failed", e); }
    }
    ...
    return rows.map((r) => {
      ...
      if (!expiresAt) { const fallback = new Date(r.created_at); fallback.setDate(fallback.getDate() + (codeRow?.duration_days ?? 30)); expiresAt = fallback.toISOString(); }
      return { id: r.id, code: codeRow?.code ?? "—", plan: codeRow?.plan ?? "—", durationDays: codeRow?.duration_days ?? 30, note: codeRow?.note ?? null, userId: r.user_id, userEmail: emailById.get(r.user_id) ?? null, device: r.device_fingerprint, redeemedAt: r.created_at, subscriptionExpiresAt: expiresAt };
    });
  });
